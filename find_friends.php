...
union
select add_profile.*,'ADD FRIEND'  as friend_status from add_profile where reg_id not in(select req_id from friend_request where frnd_id='$id') and reg_id not in (select user_id from friend_request where req_id='$id') and reg_id!=$id");
...
//                                                
                                            ?>
                                            <button type="submit" id="add_friend" type="submit" name="add_friend" style="height: 30px;min-width: 100px;background-color: #0097b9;" ><a href="find_friends.php?fid=<?php echo $row2["reg_id"] ?>" style="color: white;text-decoration: none"><?php
                                                    if ($rows["status"] == 'Friends') {
                                                        echo "Friends";
                                                    } elseif ($rows["status"] == 'Request Send') {
